...
impl RiskAssessor {
    ...
    pub fn assess(&self, operation: &Operation) -> Result<RiskTier, EngineError> {
        // First, classify the operation by type
...
mod tests {
    ...
    #[test]
    fn test_remote_escalation_tier0() {
        let assessor = RiskAssessor::new();
    ...
    #[test]
    fn test_remote_escalation_tier1() {
        let assessor = RiskAssessor::new();
    ...
    #[test]
    fn test_remote_escalation_tier2() {
        let assessor = RiskAssessor::new();
        let op = Operation::new("delete_file", vec![], OperationSource::Remote);
        let tier = assessor.assess(&op).unwrap();
        assert_eq!(tier, RiskTier::Tier2); // Already max tier
    }
