...
fn main() {
    ...
    fs::write(&dev_dest_path, &dev_key_bytes).expect("Failed to write dev public key");
...
fn load_team_public_key() -> Vec<u8> {
    ...
    if let Ok(key_hex) = env::var("ROVE_TEAM_PUBLIC_KEY") {
        if let Ok(bytes) = hex::decode(&key_hex) {
            if bytes.len() == 32 {
                println!("cargo:warning=Loaded team public key from ROVE_TEAM_PUBLIC_KEY env var");
                return bytes;
            }
        }
